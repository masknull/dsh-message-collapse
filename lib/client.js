...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var STYLE = [
      ...
      '@media (hover:hover){[data-actions-reveal="hover"] .dshmc-clock{opacity:0;transition:opacity 80ms ease}',
      '  [data-actions-reveal="hover"]:hover .dshmc-clock,[data-actions-reveal="hover"]:focus-within .dshmc-clock{opacity:1}}',
    ].join('\n')
    ...
    // ── icon set ─────────────────────────────────────────────────────────
    // The primitives package ships these glyphs under `…Regular` / `…Medium`
    // names only. An earlier revision referenced `IconChevronDownOutline14` /
    // `IconCheckOutline16` / `IconCopyOutline16`, none of which exist in the
    // export table of either host line, so `React.createElement(undefined)`
    // threw inside the slot renderer and the whole row fell back to the
    // product's default bubble (the collapse feature silently disappeared).
    // `Regular` is the 14px one-pixel stroke variant this plugin draws at 14/16
    // px via CSS, so it is a drop-in.
    var IconChevronDown = prim.IconChevronDownOutline14 || prim.IconChevronDownOutlineRegular
    var IconCheck = prim.IconCheckOutline16 || prim.IconCheckOutlineRegular
    var IconCopy = prim.IconCopyOutline16 || prim.IconCopyOutlineRegular
    ...
    function CollapsibleUserNode(props) {
      ...
      return React.createElement('div', { className: 'dshmc-row', 'data-actions-reveal': 'hover' },
